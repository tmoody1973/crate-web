  ...
  switch (cmd.toLowerCase()) {
    ...
    case "influence": {
      ...
      return [
        ...
        `   a. search_reviews — find critical reviews that mention other artists (co-mentions = influence signal)`,
        `   b. extract_influences — extract influence relationships from review text`,
        `   c. search_web (Exa) — search for "${arg} musical influences interview" for deeper analysis`,
        ...
        `3. For EVERY discovered influence, fetch tag profiles:`,
        `   a. fetch_artist_tags for ${arg} (the central artist)`,
        `   b. fetch_artist_tags for each connected artist`,
        `   c. This builds sonic similarity data for future queries`,
        `4. For EVERY artist discovered, get their image:`,
        ...
        `5. Cache all discoveries: cache_batch_influences with source citations`,
        ``,
        `DIRECTION CONVENTION (from Badillo-Goicoechea 2025):`,
        `- Edge direction: from=INFLUENCER, to=INFLUENCED`,
        `- If a review of Artist B mentions Artist A, the edge goes from A to B (A influences B)`,
        `- Co-mentions in the SAME review = co_mention relationship (bidirectional)`,
        `- Use "influenced" for clear directional influence, "co_mention" for co-occurrence in reviews`,
        `- Each source/review is one "mention" — the more reviews cite the same connection, the stronger it is`,
        ``,
        `OUTPUT FORMAT — THIS IS A HARD REQUIREMENT, NOT A SUGGESTION:`,
        `Your final output MUST be ONLY an OpenUI Lang code block. No prose before or after.`,
        `Do NOT write markdown, tables, headers, bullet points, or ASCII art.`,
        `The UI will render your component as an interactive visual — plain text will NOT render properly.`,
        ``,
        `You MUST output EXACTLY this format (one line, with the JSON connections string):`,
        ``,
        `\`\`\``,
        `root = InfluenceChain("${arg}", "[{\\"name\\":\\"Artist Name\\",\\"weight\\":0.9,\\"relationship\\":\\"influenced by\\",\\"context\\":\\"2-3 sentences on why and how\\",\\"sources\\":[{\\"name\\":\\"Pitchfork\\",\\"url\\":\\"https://...\\"}],\\"imageUrl\\":\\"https://i.scdn.co/image/...\\"}]")`,
        `\`\`\``,
        ``,
        `RULES:`,
        `- The connections prop is a JSON array encoded as a string (with escaped quotes)`,
        `- Include 8-15 connections sorted by weight descending`,
        `- Every connection MUST have an imageUrl from search_spotify_artwork`,
        `- Every connection MUST have at least one source with a real URL`,
        `- context should be 2-3 researched sentences, not generic descriptions`,
        `- Output ONLY the OpenUI Lang block. No surrounding text, no introduction, no summary.`,
      ].join("\n");
    }
